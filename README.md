...
| :--- | :--- |
| USE_S3 | If 'TRUE' then static files are served from Amazon S3. If 'FALSE' static files are served by the Django development server. |
...
| DISABLE_COLLECTSTATIC | Set to 1 to stop Heroku running collectstatic during the deployment as the static files will be served from Amazon S3. |
